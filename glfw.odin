...
Mouse_Button_Proc     :: #type proc "c" (window: Window_Handle, button: Mouse_Button, action: Action, mods: i32);
...
Joystick_Proc         :: #type proc "c" (joy: Joystick, event: i32);
...
GetKey :: inline proc(window: Window_Handle, key: Key) -> bool {
    foreign glfw {
        glfwGetKey :: proc "c" (window: Window_Handle, key: Key) -> i32 ---;
    }
    return glfwGetKey(window, key) != 0;
}
GetKeyName :: proc(key: Key, scancode: i32) -> string {
    foreign glfw {
        glfwGetKeyName :: proc "c" (key: Key, scancode: i32) -> ^u8 ---;
    }
