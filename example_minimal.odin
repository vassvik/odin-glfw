...
error_callback :: proc(error: i32, desc: ^u8) #cc_c {
    fmt.printf("Error code glfw %d:\n    %s\n", error, strings.to_odin_string(desc));
}

error_callback_ :: proc(error: int, desc: string) {
    fmt.printf("Error code odin %d:\n    %s\n", error, desc);
}
...
main :: proc() {
    ...
    str := "asdasd";
    error_callback_(123, str);
    error_callback(123, &str[0]);
    ...
    title := "blah\x00";
    window := glfw.create_window(1280, 720, &title[0], nil, nil);
    if window == nil {
        glfw.Terminate();
        return;
    }
